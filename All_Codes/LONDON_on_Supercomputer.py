...
t= tuple((np.around(i,2),0,0) for i in np.around(np.linspace(0,0.1,10),2))
...
#for script file update 
    scr[23]=f'/home/sangita.sen/packages/london/london.x file_x'+str(i)+'.inp >file_x'+str(i)+'.out\n'
    script1=open("h2o_x_"+str(i)+".sh","w")
    for l in scr:
        script1.write(l)
    script1.close()
    ...
    os.system("qsub h2o_x_"+str(i)+".sh")
f.close() 
