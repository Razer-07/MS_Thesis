...
t= tuple((0,np.around(i,2),0) for i in np.around(np.linspace(0,0.1,10),2))


#Loading the given input file of H20
f=open('h2o.UHF.0.000.inp')
script=open('h2o.sh')
with script as line:
	scr=line.readlines()
with f as line:
...
#for script file update 
    scr[23]=f'/home/sangita.sen/packages/london/london.x file_y'+str(i)+'.inp >file_y'+str(i)+'.out\n'
    script1=open("h2o_y_"+str(i)+".sh","w")
    for l in scr:
        script1.write(l)
    script1.close()
    ...
    os.system("qsub h2o_y_"+str(i)+".sh")
f.close() 
